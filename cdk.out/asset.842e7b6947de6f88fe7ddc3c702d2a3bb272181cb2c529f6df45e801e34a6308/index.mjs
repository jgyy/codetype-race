...
function withHttp(inputSchema, handler2) {
  return async (event) => {
    ...
    try {
      ...
      const out = await handler2(input, {
        requestId,
        route,
        userId,
        groups,
...
var DailyRepo = class {
  ...
  async submitBest(date, userId, displayName, scaledWpm2) {
    const existing = await this.getUser(date, userId);
    if (existing && existing.scaled_wpm >= scaledWpm2) {
      return { improved: false, bestWpm: existing.scaled_wpm };
    }
    const finishedAt = Date.now();
    const newRunSK = dailyRunSK(scaledWpm2, userId);
    const transactItems = [];
    ...
    return { improved: true, bestWpm: scaledWpm2 };
  }
...
var SnippetRepo = class {
  ...
  async approveOrReject(snippetId, reviewerId, decision, reason) {
    ...
    try {
      await this.client.send(
        new TransactWriteCommand2({ TransactItems: transactItems })
      );
    } catch (e) {
...
import {
  ...
  TransactWriteCommand as TransactWriteCommand3,
...
var UserRepo = class {
  ...
  async applyRaceResults(roomId, language, participants) {
    ...
    await this.client.send(new TransactWriteCommand3({ TransactItems: items }));
    return applied;
  }
  ...
  async applyDecayToProfile(userId, newRating, seasonId) {
    try {
      ...
      if (e instanceof ConditionalCheckFailedException3) return false;
      throw e;
    }
  }
};
...
var handler = withHttp(
  ...
  async (input, ctx) => {
    ...
    const runs = await daily.listRuns(input.date, 1e3);
    const rank = runs.findIndex((r) => r.user_id === ctx.userId) + 1 || runs.length + 1;
