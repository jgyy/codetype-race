...
var RoomStatusSchema = external_exports.enum([
  "lobby",
  "countdown",
  "running",
  "finished"
]);
var RoleSchema = external_exports.enum(["racer", "spectator"]);
var RoomCodeSchema = external_exports.string().length(6).transform((s) => s.toUpperCase());
var DisplayNameSchema = external_exports.string().trim().regex(/^[A-Za-z0-9 _-]{1,24}$/, "invalid display_name");
var RoomSchema = external_exports.object({
  ...
  snippet_id: external_exports.string(),
  status: RoomStatusSchema,
  created_at: external_exports.number(),
  started_at: external_exports.number().optional(),
  finished_at: external_exports.number().optional(),
...
var PlayerSchema = external_exports.object({
  ...
  joined_at: external_exports.number(),
  finished_at: external_exports.number().optional(),
  gross_wpm: external_exports.number().optional(),
  net_wpm: external_exports.number().optional(),
  accuracy: external_exports.number().optional(),
  scaled_wpm: external_exports.number().optional(),
  chars_typed: external_exports.number(),
  errors: external_exports.number(),
  progress: external_exports.number(),
  is_dnf: external_exports.boolean().optional(),
...
var DifficultySchema = external_exports.number().int().min(1).max(5);
var SnippetStatusSchema = external_exports.enum(["pending", "approved", "rejected"]);
var SnippetSchema = external_exports.object({
  ...
  reviewed_by: external_exports.string().optional(),
  reviewed_at: external_exports.number().optional(),
  reject_reason: external_exports.string().optional()
});
var SnippetSubmissionSchema = external_exports.object({
  language: external_exports.string().min(1).max(40),
  difficulty: DifficultySchema,
  title: external_exports.string().min(3).max(80),
  text: external_exports.string().min(20).max(2e3),
  source: external_exports.string().url().optional()
});
var SnippetActionSchema = external_exports.object({
  snippet_id: external_exports.string().min(1),
  reason: external_exports.string().max(280).optional()
});
var ListPendingResponseSchema = external_exports.object({
  items: external_exports.array(SnippetSchema.passthrough())
});
var SnippetFiltersSchema = external_exports.object({
  language: external_exports.string().optional(),
  difficulty: DifficultySchema.optional()
});
var CreateRoomRequestSchema = external_exports.object({
  snippet_id: external_exports.string().min(1).optional(),
  filters: SnippetFiltersSchema.optional(),
  previous_room_id: external_exports.string().min(1).optional(),
  new_snippet: external_exports.boolean().optional()
}).refine(
  (v) => v.snippet_id || v.filters || v.previous_room_id,
  { message: "snippet_id, filters, or previous_room_id is required" }
);
var CreateRoomResponseSchema = external_exports.object({
  room_id: external_exports.string(),
  code: external_exports.string().length(6)
});
var JoinRoomRequestSchema = external_exports.object({
  code: RoomCodeSchema,
  display_name: DisplayNameSchema,
  role: RoleSchema.optional().default("racer")
});
var JoinRoomResponseSchema = external_exports.object({
  room_id: external_exports.string(),
  snippet_id: external_exports.string(),
  status: RoomStatusSchema
});
var GetRoomPathSchema = external_exports.object({
  code: RoomCodeSchema
});
var GetRoomResponseSchema = external_exports.object({
  room_id: external_exports.string(),
  code: external_exports.string(),
  snippet_id: external_exports.string(),
  status: RoomStatusSchema,
...
var HistoryEntrySchema = external_exports.object({
  room_id: external_exports.string().optional(),
  display_name: external_exports.string().optional(),
  snippet_id: external_exports.string().optional(),
  mode: external_exports.enum(["race", "practice"]).optional(),
  finished_at: external_exports.number(),
  gross_wpm: external_exports.number(),
  net_wpm: external_exports.number(),
  accuracy: external_exports.number(),
  scaled_wpm: external_exports.number(),
  chars_typed: external_exports.number(),
  errors: external_exports.number()
});
var ReplaySchema = external_exports.object({
  version: external_exports.literal(1),
  room_id: external_exports.string(),
  snippet_id: external_exports.string(),
  started_at: external_exports.number(),
  ...
  participants: external_exports.array(
    external_exports.object({
      ...
      samples: external_exports.array(external_exports.tuple([external_exports.number(), external_exports.number()]))
    })
  )
});
var ReplayUploadUrlResponseSchema = external_exports.object({
  upload_url: external_exports.string().url(),
  key: external_exports.string()
});
var ReplayResponseSchema = external_exports.object({
  download_url: external_exports.string().url(),
  key: external_exports.string()
});
var PracticeRunRequestSchema = external_exports.object({
  snippet_id: external_exports.string().min(1),
  chars_typed: external_exports.number().int().min(1),
  errors: external_exports.number().int().min(0),
  duration_ms: external_exports.number().int().min(1),
  save: external_exports.boolean().optional().default(true)
});
var UserProfileSchema = external_exports.object({
  user_id: external_exports.string(),
  display_name: external_exports.string(),
  rating: external_exports.number().int(),
  races_completed: external_exports.number().int(),
  races_won: external_exports.number().int(),
  best_wpm: external_exports.record(external_exports.string(), external_exports.number()),
  created_at: external_exports.number()
});
var RaceHistoryEntrySchema = external_exports.object({
  room_id: external_exports.string(),
  finished_at: external_exports.number(),
  display_name: external_exports.string(),
  language: external_exports.string().optional(),
  ...
  gross_wpm: external_exports.number(),
  accuracy: external_exports.number(),
  rating_delta: external_exports.number(),
  rating_after: external_exports.number()
});
var GetUserResponseSchema = external_exports.object({
  profile: UserProfileSchema,
  recent: external_exports.array(RaceHistoryEntrySchema.passthrough()),
  groups: external_exports.array(external_exports.string()).optional()
});
var LeaderboardEntrySchema = external_exports.object({
  user_id: external_exports.string(),
  display_name: external_exports.string(),
  rating: external_exports.number()
});
var GetLeaderboardResponseSchema = external_exports.object({
  entries: external_exports.array(LeaderboardEntrySchema)
});
var DailyMetaSchema = external_exports.object({
  date: external_exports.string(),
  snippet_id: external_exports.string(),
  selected_at: external_exports.number()
});
var GetDailyResponseSchema = external_exports.object({
  date: external_exports.string(),
  snippet: SnippetSchema
});
var DailySubmitRequestSchema = external_exports.object({
  date: external_exports.string(),
  snippet_id: external_exports.string(),
  chars_typed: external_exports.number().int().min(1),
  errors: external_exports.number().int().min(0),
...
var DailySubmitResponseSchema = external_exports.object({
  improved: external_exports.boolean(),
  best_wpm: external_exports.number(),
  rank: external_exports.number().int().min(1)
});
var DailyLeaderboardEntrySchema = external_exports.object({
  user_id: external_exports.string(),
  display_name: external_exports.string(),
  scaled_wpm: external_exports.number(),
  finished_at: external_exports.number()
});
var GetDailyLeaderboardResponseSchema = external_exports.object({
  date: external_exports.string(),
  entries: external_exports.array(DailyLeaderboardEntrySchema)
});
var PracticeRunResponseSchema = external_exports.object({
  finished_at: external_exports.number(),
  gross_wpm: external_exports.number(),
  net_wpm: external_exports.number(),
  accuracy: external_exports.number(),
...
var ListHistoryResponseSchema = external_exports.object({
  results: external_exports.array(HistoryEntrySchema.passthrough())
});
var WsCursorSchema = external_exports.object({
  action: external_exports.literal("cursor"),
  progress: external_exports.number(),
  chars_typed: external_exports.number().int(),
  errors: external_exports.number().int().min(0)
});
var WsPingSchema = external_exports.object({ action: external_exports.literal("ping") });
var WsStartSchema = external_exports.object({ action: external_exports.literal("start") });
var WsFinishSchema = external_exports.object({
  action: external_exports.literal("finish"),
  chars_typed: external_exports.number().int().min(0),
  errors: external_exports.number().int().min(0)
});
var WsChatSchema = external_exports.object({
  action: external_exports.literal("chat"),
  text: external_exports.string().min(1).max(280)
});
var WsClientMsgSchema = external_exports.discriminatedUnion("action", [
  WsCursorSchema,
  WsPingSchema,
  WsStartSchema,
  WsFinishSchema,
...
var WsConnectQuerySchema = external_exports.object({
  code: RoomCodeSchema,
  display_name: DisplayNameSchema,
  role: RoleSchema.optional().default("racer")
});
var WsServerCursorSchema = external_exports.object({
  type: external_exports.literal("cursor"),
  display_name: external_exports.string(),
  progress: external_exports.number()
});
var WsServerStartSchema = external_exports.object({
  type: external_exports.literal("start"),
  started_at: external_exports.number()
});
var WsServerFinishSchema = external_exports.object({
...
var WsServerRoomEventSchema = external_exports.object({
  type: external_exports.literal("room-event"),
  event: external_exports.enum(["join", "leave", "status"]),
  payload: external_exports.unknown()
});
var WsServerChatSchema = external_exports.object({
  type: external_exports.literal("chat"),
  display_name: external_exports.string(),
  text: external_exports.string(),
  ts: external_exports.number()
});
var WsServerRatingsSchema = external_exports.object({
  type: external_exports.literal("ratings"),
  entries: external_exports.array(
    external_exports.object({
      user_id: external_exports.string(),
      display_name: external_exports.string(),
      delta: external_exports.number().int(),
      rating_after: external_exports.number().int()
    })
  )
});
var WsServerKickedSchema = external_exports.object({ type: external_exports.literal("kicked") });
var WsServerErrorSchema = external_exports.object({
  type: external_exports.literal("error"),
  code: external_exports.string(),
  message: external_exports.string()
});
var WsServerMsgSchema = external_exports.discriminatedUnion("type", [
...
function withWs(inputSchema, handler2) {
  return async (event) => {
    ...
    try {
      ...
      await handler2(input, { connectionId, requestId, routeKey });
...
var pendingQueuePK = () => "QUEUE#SNIPPETS#PENDING";
var pendingQueueSK = (submittedAt, snippetId) => `SUBMITTED#${submittedAt}#${snippetId}`;
...
var langGSI1PK = (language) => `LANG#${language}`;
var snippetDiffGSI1SK = (difficulty, snippetId) => `DIFF#${difficulty}#SNIPPET#${snippetId}`;
var snippetDiffPrefix = (difficulty) => `DIFF#${difficulty}#`;
...
async function flush() {
  ...
  await Promise.all(
    Array.from(snapshot.entries()).map(async ([connectionId, state]) => {
      ...
      await Promise.all(
        peers.filter((id) => id !== connectionId).map((id) => postTo(id, payload).catch(() => false))
      );
    })
  );
}
async function applyCursor(input, connectionId) {
...
// ws/heartbeat.ts
async function applyHeartbeat(connectionId) {
  const conn = await connections.byConnectionId(connectionId);
  if (!conn) throw Errors.NotFound("connection");
  const roomId = conn.PK.slice("ROOM#".length);
...
async function applyStart(connectionId) {
  const conn = await connections.byConnectionId(connectionId);
  if (!conn) throw Errors.NotFound("connection");
  const roomId = conn.PK.slice("ROOM#".length);
...
function computeRatingDeltas(participants, k = DEFAULT_K) {
  const out = {};
...
function computeWpm(charsTyped, durationMs) {
  if (durationMs <= 0) return 0;
  return charsTyped / 5 / (durationMs / 6e4);
}
function evaluateStats(stats, thresholds = {}) {
  ...
  if (stats.stddevIntervalMs !== void 0 && (stats.totalKeystrokes ?? 0) > t.minKeystrokesForVariance && stats.stddevIntervalMs < t.minStddevMs) {
    out.push({
      code: "LOW_VARIANCE",
      severity: "medium",
  ...
  if (stats.maxPosAdvance !== void 0 && stats.maxPosAdvance > t.maxPosAdvance) {
    out.push({
...
function isFlagged(signals) {
  return signals.some((s) => s.severity === "high");
}

// src/repos/SnippetRepo.ts
import {
...
var RANDOM_PAGE_SIZE = 25;
var SnippetRepo = class {
  constructor(client2 = ddb, rng = Math.random) {
    this.client = client2;
    this.rng = rng;
  }
  async getById(snippetId) {
    const r = await this.client.send(
      new GetCommand2({
        TableName: TABLE,
  ...
  async list(filters = {}, limit = 100) {
    if (!filters.language) {
      const r2 = await this.client.send(
        new ScanCommand({
          TableName: TABLE,
          FilterExpression: "begins_with(PK, :p) AND SK = :sk AND (attribute_not_exists(#status) OR #status = :approved)",
    ...
    const r = await this.client.send(
      new QueryCommand3({
        ...
        FilterExpression: "attribute_not_exists(#status) OR #status = :approved",
        ExpressionAttributeNames: { "#status": "status" },
  ...
  async submitPending(snippetId, submittedBy, submission) {
  ...
  async approveOrReject(snippetId, reviewerId, decision, reason) {
    ...
    const submittedAt = snippet.submitted_at;
    const transactItems = [
      {
        Update: {
          TableName: TABLE,
          Key: { PK: snippetPK(snippetId), SK: "META" },
          UpdateExpression: "SET #status = :s, reviewed_by = :rb, reviewed_at = :ra" + (decision === "rejected" && reason ? ", reject_reason = :rr" : ""),
  ...
  async incrementDailySubmitCounter(userId, date, limit = 5) {
    try {
      const r = await this.client.send(
        new UpdateCommand3({
          TableName: TABLE,
          ...
          ExpressionAttributeValues: {
            ":one": 1,
            ":limit": limit,
            ":ttl": Math.floor(Date.now() / 1e3) + 172800
          },
...
import {
  ...
  TransactWriteCommand as TransactWriteCommand2,
  UpdateCommand as UpdateCommand4
} from "@aws-sdk/lib-dynamodb";
import { ConditionalCheckFailedException as ConditionalCheckFailedException4 } from "@aws-sdk/client-dynamodb";
...
var UserRepo = class {
  ...
  async getOrCreate(userId, displayName) {
    ...
    try {
      ...
      if (e instanceof ConditionalCheckFailedException4) {
  ...
  async applyRaceResults(roomId, language, participants) {
    ...
    await this.client.send(new TransactWriteCommand2({ TransactItems: items }));
    return applied;
  }
  ...
  async applyDecayToProfile(userId, newRating, seasonId) {
    try {
      ...
      if (e instanceof ConditionalCheckFailedException4) return false;
      throw e;
    }
  }
};
...
// ws/finish.ts
async function applyFinish(input, connectionId) {
  const conn = await connections.byConnectionId(connectionId);
  if (!conn) throw Errors.NotFound("connection");
...
async function maybeApplyRatings(roomId, language) {
  ...
  const finishedRacers = racers.filter(
    (p) => p.finished_at !== void 0 && !p.is_dnf
  );
  if (finishedRacers.length !== racers.length) return;
  const ranked = [...finishedRacers].sort(
    (a, b) => (b.scaled_wpm ?? 0) - (a.scaled_wpm ?? 0) || (a.finished_at ?? 0) - (b.finished_at ?? 0)
  );
  const rated = ranked.filter((p) => !!p.user_id).map((p, i) => ({
    player: p,
    finishOrder: i + 1
  }));
...
// ws/chat.ts
async function applyChat(input, connectionId) {
  const conn = await connections.byConnectionId(connectionId);
  if (!conn) throw Errors.NotFound("connection");
  const roomId = conn.PK.slice("ROOM#".length);
  const room = await rooms.getMeta(roomId);
...
// ws/default.ts
var handler = withWs(WsClientMsgSchema, async (input, ctx) => {
  switch (input.action) {
    case "cursor":
