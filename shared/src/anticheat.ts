// Anti-cheat heuristics. **Flag, never auto-ban.** Every signal is
// advisory; downstream code chooses how to apply it (skip rating,
// exclude from leaderboard, badge the run, etc.).
//
// Two entry points:
//   - evaluateStats() takes pre-computed aggregate stats. Used on the
//     wire (finish.ts, daily submit) where keystroke samples aren't
//     available cheaply.
//   - evaluateRun() takes raw keystroke samples and runs the full
//     analysis (variance, paste detection). Used in tests today; will
//     be used by the replay pipeline once samples are persisted.
...
export interface KeystrokeSample {
  /** wall-clock or relative ms; only deltas matter. */
  t: number;
  /** char (or empty string for backspace). */
  char: string;
}
...
export interface RunStats {
  ...
  durationMs: number;
  charsTyped: number;
  /** Mean ms between successive keystrokes; omit if no sample buffer. */
  meanIntervalMs?: number;
  /** Std dev of those intervals. */
  stddevIntervalMs?: number;
  /** Largest gap between two successive keystrokes. */
  maxIntervalGapMs?: number;
  /**
   * Largest single-frame advance in `pos` from a sample stream — used
   * as a paste indicator. `undefined` when sample data isn't available.
   */
...
export function statsFromSamples(
  ...
  // Rough paste indicator: detect bursts where many chars land within
  // a 50ms window. The "pos advance" is the count of chars in the
  // tightest such burst.
  let maxPosAdvance = 0;
  let burst = 1;
  for (let i = 1; i < samples.length; i++) {
...
/** Highest severity present, or null if no signals. */
...
/** Convenience: a run is "flagged" once any high-severity signal fires. */
