...
describe("evaluateStats — wire heuristics", () => {
  ...
  test("WPM_TOO_HIGH at 300 wpm", () => {
    const sigs = evaluateStats({
      snippetLength: 200,
      durationMs: 20_000, // 200/5/(20/60) = 120 wpm — not flagged
      charsTyped: 200,
    });
    expect(sigs).toEqual([]);
...
describe("evaluateRun — fixtures", () => {
  test("synthetic bot (constant 40ms) flags WPM_TOO_HIGH and LOW_VARIANCE", () => {
    // 200 keystrokes × 40ms = 8s → 300 wpm; zero stddev triggers
    // LOW_VARIANCE; the tight intervals also fire PASTE_SUSPECTED.
  ...
  test("fast human (mean 100ms, lots of jitter) does NOT flag", () => {
    ...
    let seed = 0;
    for (let i = 0; i < 200; i++) {
      // pseudo-random jitter 60..160ms — ~150 wpm with natural variance
  ...
  test("paste burst flags PASTE_SUSPECTED", () => {
    // 20 chars within a single 50ms burst, then normal typing.
