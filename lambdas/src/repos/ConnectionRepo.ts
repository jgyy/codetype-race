...
export class ConnectionRepo {
  ...
  /**
   * Token-bucket-ish counter on the connection row. Read current state,
   * decide what the next state should be (resetting if the window has
   * elapsed), and write it back conditionally on the read values so a
   * second concurrent chat doesn't slip past the cap. Surface
   * RATE_LIMITED if the cap would be exceeded or another writer raced
   * us.
   */
