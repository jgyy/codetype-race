...
/**
 * GET /snippets/starter-pack?languages=ts,py,go&n=30
 *
 * Returns up to N approved snippets, sampled across the requested
 * languages, for offline prefetch by the PWA service worker (Phase 12).
 * No auth: this hits CloudFront with a long TTL; the response body is
 * identical for any user.
 */
...
export const handler = withHttp(EmptyBody, async (_input, ctx) => {
  ...
  // Per-language fetch when filtered; otherwise one unfiltered list call.
  ...
  // Deterministic dedupe by snippet_id, then truncate.
